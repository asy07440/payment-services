<template>
  ...
  >
    <v-card>
      ...
      <v-card-actions class="px-5 py-5">
        ...
        <v-btn color="#365B97" variant="flat" @click="saveDepartemen">
          Simpan
        </v-btn>
      </v-card-actions>
    </v-card>
  ...
  >
    <v-card>
      ...
      <v-card-text class="pa-8">
        <a-text-field
          v-model="editDept.nama"
          label="Nama Departemen"
          class="mb-3"
        />

        <a-text-field
          v-model="editDept.keterangan"
          label="Keterangan (optional)"
          class="mb-3"
        />
      </v-card-text>
      ...
      <v-card-actions class="px-5 py-5">
        ...
        <v-btn color="#365B97" variant="flat" @click="ubahDepartemen">
          Edit
        </v-btn>
      </v-card-actions>
    </v-card>
    ...
    <v-col cols="9">
      <v-breadcrumbs>
        <v-breadcrumbs-item active>
          <span class="font-weight-medium text-h5"> Master Departemen </span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  ...
  <v-row>
    <v-col>
      <v-card flat rounded="lg" class="border">
        ...
        <v-data-table
          :headers="data.headDept"
          :items="deptStore.getDataDepartemen"
          :search="data.search"
        >
          ...
          <template v-slot:item.aksi="{ item }">
            <v-row justify="center" align="center">
              ...
              <v-btn color="red" size="20" @click="hapusDepartemen(item.id!)">
...
import { useMasterDepartemenStore } from "~/stores/perusahaan/departemenStore";

definePageMeta({
  layout: "perusahaan",
});

const deptStore = useMasterDepartemenStore();
...
onMounted(async () => {
  await deptStore.tarikDataDepartemenAct();
});

const newDept = ref<departemenM>({
  id: "",
  nama: "",
  keterangan: "",
  id_perusahaan: "",
...
const editDept = ref<departemenM>({
  id: "",
  nama: "",
  keterangan: "",
  id_perusahaan: "",
...
const data = reactive({
  ...
  headDept: [
    { title: "No", value: "no" },
    { title: "Nama Departemen", value: "nama", sortable: true },
...
const ubahDepartemen = async () => {
  const d = editDept.value;
  ...
  deptStore.updateDepartemenAct(d);
  data.dialogEdit = false;
};

const hapusDepartemen = async (id: string) => {
  const confirmed = await confirmationDialog.value?.show(
    "Konfirmasi Hapus",
  ...
  deptStore.deleteDepartemenAct(id);
};
